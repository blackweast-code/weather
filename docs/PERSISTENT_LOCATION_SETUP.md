...
이 프로젝트는 오전 8시 ChatGPT 자동화가 브라우저 쿠키 없이 `/api/weather/current`를 호출합니다. 따라서 마지막으로 저장한 관리자 휴대폰 위치를 안정적으로 쓰려면 Vercel에 영구 저장소를 연결해야 합니다.
...
5. 오전 8시 자동화가 `/api/weather/current`를 호출하면 서버가 KV/Upstash Redis의 마지막 위치를 먼저 읽고, 그 위치 기준으로 날씨를 계산합니다.
...
배포 후 아래 주소를 열었을 때 `storage` 값이 `persistent`이고 `persisted`가 `true`이면, 오전 8시 자동화도 마지막 저장 위치를 사용합니다.
